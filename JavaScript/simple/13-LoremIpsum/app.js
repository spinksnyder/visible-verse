...
form.addEventListener("submit", function (event) {
  ...
  const numberOfParagraphs = parseInt(document.getElementById("amount").value);

  // if number is 0 or -ve or null then return 1 paragraph
  let paragraphs = "";
  if (
    isNaN(numberOfParagraphs) ||
    numberOfParagraphs <= 0 ||
    numberOfParagraphs > 10
  ) {
    paragraphs = getParagraphs(getRandomNumber(), true);
  } else {
    paragraphs = getParagraphs(numberOfParagraphs, false);
  }
...
function getParagraphs(number, doesReturnSingleParagraph) {
  let newText = "";
  if (doesReturnSingleParagraph) {
    newText = decorateParagraph(text.slice(number, number + 1));
  } else {
    const aText = text.slice(0, number);
    newText = aText
      .map(function (item) {
        return decorateParagraph(item);
      })
      .join("");
  }
  return newText;
...
function decorateParagraph(item) {
